...
export default function HandlerView() {
  ...
  const handleSearchTracking = async (e: React.FormEvent) => {
    ...
    try {
      const q = query(collection(db, 'shipments'), where('trackingNumber', '==', trackingCode.trim().toUpperCase()));
      const snap = await getDocs(q);
      
      if (snap.empty) {
        setSearchError('No shipment found with this intelligence code.');
      } else {
        const shipmentData = { id: snap.docs[0].id, ...snap.docs[0].data() };
        setSelectedShipment(shipmentData);
        setTrackingCode('');
      }
    } catch (err) {
      console.error("Search error:", err);
      setSearchError('Intelligence extraction failed. System offline?');
    } finally {
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="relative">
        <div className="absolute inset-0 opacity-[0.02] pointer-events-none overflow-hidden rounded-[2rem]">
          <img 
      ...
      <div className="bg-brand-black-light p-8 rounded-[2rem] border border-white/5 shadow-2xl relative overflow-hidden group">
        <div className="absolute inset-0 bg-brand-yellow/[0.02] opacity-0 group-hover:opacity-100 transition-opacity" />
        <div className="relative z-10">
          <div className="flex flex-wrap items-center justify-between gap-8">
            <div className="max-w-md">
              <h2 className="text-xl font-black text-white uppercase tracking-tight mb-2">Intelligence Retrieval</h2>
              <p className="text-slate-500 text-sm font-medium">Enter a tracking code to reveal full shipment manifest and logistics intel.</p>
            </div>
            <form onSubmit={handleSearchTracking} className="flex-grow max-w-xl flex gap-4">
              <div className="flex-grow relative">
                <Shield className="absolute left-4 top-1/2 -translate-y-1/2 text-brand-yellow" size={18} />
                <input 
                  type="text" 
                  placeholder="ENTER TRACKING CODE (e.g. FP-UG-XXXXXX)"
                  value={trackingCode}
                  onChange={(e) => setTrackingCode(e.target.value.toUpperCase())}
                  className="w-full pl-12 pr-6 py-4 bg-white/5 border border-white/10 rounded-2xl outline-none focus:ring-2 focus:ring-brand-yellow/50 text-white font-black uppercase tracking-widest text-xs"
                />
              </div>
              <button 
                type="submit"
                disabled={searchLoading}
                className="px-8 py-4 bg-brand-yellow text-brand-black font-black uppercase tracking-widest text-[10px] rounded-2xl shadow-lg shadow-brand-yellow/10 hover:bg-brand-yellow-light transition-all disabled:opacity-50 flex items-center gap-2"
              >
                {searchLoading ? <div className="w-4 h-4 border-2 border-brand-black border-t-transparent rounded-full animate-spin" /> : <ArrowRight size={16} />}
                Retrieve Intel
              </button>
            </form>
          </div>
          {searchError && (
            <motion.p 
              ...
              animate={{ opacity: 1, y: 0 }}
              className="mt-4 text-red-500 text-[10px] font-black uppercase tracking-widest flex items-center gap-2"
            >
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-brand-black-light p-8 rounded-[2rem] border border-white/5 shadow-2xl space-y-8">
          ...
          <div className="grid grid-cols-1 gap-6">
            {assignedShipments.length > 0 ? assignedShipments.map((shipment, i) => (
              <motion.div 
                ...
                transition={{ delay: i * 0.1 }}
                className="bg-white/5 p-6 rounded-[1.5rem] border border-white/5 shadow-2xl hover:border-brand-yellow/30 transition-all group relative overflow-hidden"
              >
                <div className="absolute inset-0 opacity-[0.02] group-hover:opacity-[0.05] transition-opacity duration-700 pointer-events-none">
                  <img 
                    src={
                      shipment.status === 'Delivered' ? 'https://images.unsplash.com/photo-1542744094-24638eff58bb?q=80&w=300&h=300&auto=format&fit=crop' :
                      'https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?q=80&w=300&h=300&auto=format&fit=crop'
                    } 
                    alt="Shipment pattern" 
                    className="w-full h-full object-cover"
                    referrerPolicy="no-referrer"
                  />
                </div>
                <div className="relative z-10 flex flex-wrap justify-between items-center gap-6 mb-8">
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          <div className="bg-brand-black-light p-8 rounded-[2rem] border border-white/5 shadow-2xl relative overflow-hidden">
            <div className="absolute inset-0 opacity-[0.02] pointer-events-none">
              <img 
                src="https://images.unsplash.com/photo-1553413077-190dd305871c?q=80&w=300&h=500&auto=format&fit=crop" 
          ...
          {/* Support Node */}
          <div className="bg-brand-yellow p-8 rounded-[2rem] shadow-2xl relative overflow-hidden group">
        ...
        {selectedShipment && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-6">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-2xl bg-brand-black-light rounded-[2rem] shadow-2xl overflow-hidden border border-white/10"
            >
        ...
        {showReportForm && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-6">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="relative w-full max-w-xl bg-brand-black-light rounded-[2rem] shadow-2xl overflow-hidden border border-white/10"
            >
              {reportSuccess ? (
                ...
                <>
                  ...
                  <form onSubmit={handleSendReport} className="p-8 space-y-8">
                    <div>
