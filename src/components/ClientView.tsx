...
import { Package, TrendingUp, Clock, CheckCircle2, Info, X, Shield, MapPin, ArrowRight, Weight, Plus, Users, ShoppingBag } from 'lucide-react';
    ...
    <div className="space-y-8 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
          {[
            ...
            <motion.button 
              ...
              className={`text-left bg-brand-black-light p-6 rounded-[2rem] border transition-all ${statusFilter === stat.filter ? 'border-brand-yellow shadow-[0_0_30px_rgba(250,204,21,0.1)]' : 'border-white/5 shadow-2xl hover:border-white/20'}`}
            >
      ...
      <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="grid grid-cols-1 gap-6">
            {filteredShipments.length > 0 ? filteredShipments.map((shipment, i) => (
              <motion.div 
                ...
                className="bg-white/5 p-6 rounded-[1.5rem] border border-white/5 shadow-2xl hover:border-brand-yellow/30 transition-all group cursor-pointer relative overflow-hidden"
              >
                <div className="absolute inset-0 opacity-[0.02] group-hover:opacity-[0.05] transition-opacity duration-700 pointer-events-none">
                  <img 
                    src={
                      shipment.status === 'Delivered' ? 'https://images.unsplash.com/photo-1578575437130-527eed3abbec?q=80&w=300&h=300&auto=format&fit=crop' :
                      'https://images.unsplash.com/photo-1542744094-24638eff58bb?q=80&w=300&h=300&auto=format&fit=crop'
                    } 
              ...
              <div className="bg-white/5 rounded-[2.5rem] p-20 border border-white/5 text-center shadow-2xl relative overflow-hidden group">
                <div className="absolute inset-0 opacity-[0.03] pointer-events-none group-hover:opacity-[0.06] transition-opacity duration-700">
                  <img 
                ...
                <div className="relative z-10">
                  ...
                  <button 
                    ...
                    className="px-8 py-4 bg-brand-yellow text-brand-black font-black uppercase tracking-widest text-xs rounded-2xl hover:bg-brand-yellow-light transition-all shadow-xl shadow-brand-yellow/20"
                  >
        ...
        <div className="lg:col-span-1 space-y-8">
          {/* Quick Actions Node */}
          <div className="bg-brand-black-light p-8 rounded-[2rem] border border-white/5 shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <button 
                className="w-full p-6 bg-green-500/10 border border-green-500/20 text-green-500 rounded-2xl flex items-center justify-between group hover:bg-green-500/20 transition-all"
              >
                <div className="flex items-center gap-4">
                  <ShoppingBag size={20} />
                  <span className="font-black uppercase tracking-widest text-xs">Elite Marketplace</span>
                </div>
                <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </button>
            </div>
