                ...
                {group.items.map((item, j) => (
                  <div key={j} className="bg-brand-black-light p-10 rounded-[2.5rem] border border-white/5 hover:border-brand-yellow/30 transition-all group relative overflow-hidden shadow-2xl">
                    <div className="absolute inset-0 opacity-[0.03] group-hover:opacity-[0.06] transition-opacity duration-700 pointer-events-none">
                      <img 
                        src={
                          group.category === 'Intra-City (Kampala)' ? 'https://images.unsplash.com/photo-1554224155-1696413575b8?q=80&w=400&h=400&auto=format&fit=crop' :
                          group.category === 'Up-Country (Nationwide)' ? 'https://images.unsplash.com/photo-1578575437130-527eed3abbec?q=80&w=400&h=400&auto=format&fit=crop' :
                          'https://images.unsplash.com/photo-1512418490979-92798ccc13b0?q=80&w=400&h=400&auto=format&fit=crop'
                        } 
                        alt={item.name} 
                        className="w-full h-full object-cover"
                        referrerPolicy="no-referrer"
